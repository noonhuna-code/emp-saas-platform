...
import { revokeSettingsSession } from "@emp/services/settings.service";

export async function POST(request: Request) {
  const route = await beginRoute();
  const endpoint = "/api/settings/sessions/revoke";
  ...
  try {
    ...
    const payload = (await request.json()) as { sessionId?: string | null };
    const session = await getServerSession();

    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await revokeSettingsSession(route.ctx!, payload.sessionId ?? "", session.sessionId);
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to revoke session", route.requestId));
  }
}
