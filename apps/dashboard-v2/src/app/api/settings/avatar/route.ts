...
import { uploadSettingsAvatar } from "@emp/services/settings.service";

export async function POST(request: Request) {
  const route = await beginRoute();
  const endpoint = "/api/settings/avatar";
  ...
  try {
    ...
    if (!(file instanceof File)) {
      return finalizeRoute(route, endpoint, jsonError("Avatar file is required", 400, route.requestId));
    }

    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await uploadSettingsAvatar(route.ctx!, file);
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to upload avatar", route.requestId));
  }
}
