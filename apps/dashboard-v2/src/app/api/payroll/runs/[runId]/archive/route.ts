...
import { archivePayrollRun } from "@emp/services/payroll.service";
...
export async function POST(
  ...
  const endpoint = "/api/payroll/runs/[runId]/archive";
  ...
  try {
    ...
    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await archivePayrollRun(route.ctx!, route.ctx!.companyId, runId.trim());
      if (!result.ok) {
        return {
          status: mapServiceErrorStatus(result.error),
          body: { ok: false, error: sanitizeServiceError(result.error, "Unable to archive payroll run") }
        };
      }
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to archive payroll run", route.requestId));
  }
}
