...
import { listLeaveApprovalHistory } from "@emp/services/leave.service";
...
export async function GET(request: Request) {
  ...
  const endpoint = "/api/leave/review/history";
  ...
  try {
    ...
    const result = await listLeaveApprovalHistory(ctx, {
      employeeId,
      status,
      dateFrom,
      dateTo,
    ...
    if (!result.ok) {
      return finalizeRoute(route, endpoint, jsonServiceError(result.error, "Approval history lookup failed", route.requestId));
    }

    return finalizeRoute(route, endpoint, NextResponse.json({ ok: true, data: result.data }, { status: 200 }));
  } catch (error) {
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to load approval history", route.requestId));
  }
}
