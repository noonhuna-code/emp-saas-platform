...
import { assignEmployeeBreak } from "@emp/services/attendance.service";
...
export async function POST(request: Request) {
  ...
  const endpoint = "/api/attendance/breaks/assign";
  ...
  try {
    ...
    const body = (await request.json()) as AssignBreakBody;

    const guarded = await runGuardedMutation(ctx, request, endpoint, async () => {
      const result = await assignEmployeeBreak(ctx, {
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to assign break", route.requestId));
  }
}
