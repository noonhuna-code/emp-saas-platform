...
import { listShiftAssignments } from "@emp/services/attendance.service";
...
export async function GET(request: Request) {
  ...
  const endpoint = "/api/attendance/shifts/assignments";
  ...
  try {
    ...
    const result = await listShiftAssignments(ctx, { employeeId, limit });
    if (!result.ok) {
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to load shift assignments", route.requestId));
  }
}
