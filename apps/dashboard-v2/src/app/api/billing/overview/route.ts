...
export async function GET() {
  ...
  const endpoint = "/api/billing/overview";
  ...
  try {
    ...
    if (!result.ok || !result.data) {
      return finalizeRoute(
        ...
        endpoint,
        jsonError(
          sanitizeServiceError(result.error, "Unable to load billing overview"),
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to load billing overview", route.requestId));
  }
}
