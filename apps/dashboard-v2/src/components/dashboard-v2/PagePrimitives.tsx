...
export const StatCard = ({
  ...
  <Card className={cn("overflow-hidden rounded-[22px] border border-slate-200/80 bg-white/94 shadow-[0_14px_34px_rgba(15,23,42,0.045)]", className)}>
    <CardContent className="space-y-2 p-3.5">
      <div className="h-1.5 w-12 rounded-full bg-[linear-gradient(90deg,rgba(37,99,235,0.92),rgba(59,130,246,0.35))]" />
      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">{label}</p>
      <div className="text-[1.45rem] font-semibold tracking-tight text-slate-950 sm:text-[1.55rem]">{value}</div>
      {hint ? <div className="text-[13px] leading-5 text-slate-600">{hint}</div> : null}
    </CardContent>
  </Card>
);
    ...
    {modules.map((module) => {
      ...
      return (
        <Link
          ...
          href={module.href}
          className="group rounded-[20px] border border-slate-200/80 bg-white/94 p-3.5 shadow-[0_12px_28px_rgba(15,23,42,0.045)] transition hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50/40 hover:shadow-[0_18px_40px_rgba(37,99,235,0.08)]"
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 space-y-2">
              <div className="inline-flex h-9 w-9 items-center justify-center rounded-[16px] border border-slate-200/80 bg-slate-50 text-slate-700 shadow-sm">
              ...
              <div className="space-y-1.5">
                ...
                <div>
                  <p className="text-sm font-semibold tracking-tight text-slate-950 sm:text-[15px]">{module.title}</p>
                  <p className="mt-1 text-[13px] leading-5 text-slate-600">{module.description}</p>
                </div>
              </div>
            </div>
            {module.metric ? (
              <Badge className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-600">
                {module.metric}
              </Badge>
            ) : null}
          </div>
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              {module.highlights.slice(0, 3).map((highlight) => (
                <Badge
                  ...
                  className="rounded-full border border-slate-200/80 bg-white/90 px-2 py-1 text-[10px] font-medium text-slate-600"
                >
                  {highlight}
                </Badge>
              ))}
          ...
          <div className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-blue-700 transition group-hover:text-blue-800">
            Open module
