...
import { getManagerDashboard } from "@emp/services/dashboard.service";
import { beginRoute, finalizeRoute } from "@/lib/server/route-helpers";
import { handleRouteError, jsonError, mapServiceErrorStatus, sanitizeServiceError } from "@/lib/server/api-errors";

export async function GET() {
  const route = await beginRoute();
  const endpoint = "/api/dashboard/manager";
  ...
  try {
    ...
    const result = await getManagerDashboard(route.ctx);
    if (!result.ok || !result.data) {
      return finalizeRoute(
        route,
        endpoint,
        jsonError(
          sanitizeServiceError(result.error, "Unable to load manager dashboard"),
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to load manager dashboard", route.requestId));
  }
}
