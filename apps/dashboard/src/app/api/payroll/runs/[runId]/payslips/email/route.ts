...
import { queuePayrollRunPayslipEmails } from "@emp/services/payroll.service";
...
export async function POST(
  ...
  const endpoint = "/api/payroll/runs/[runId]/payslips/email";
  ...
  try {
    ...
    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await queuePayrollRunPayslipEmails(route.ctx!, route.ctx!.companyId, runId.trim());
      if (!result.ok) {
        return {
          status: mapServiceErrorStatus(result.error),
          body: { ok: false, error: sanitizeServiceError(result.error, "Unable to queue payslip emails") }
        };
      }
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to queue payslip emails", route.requestId));
  }
}
