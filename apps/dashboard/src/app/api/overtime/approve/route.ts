...
import { approveOvertimeRequest } from "@emp/services/overtime.service";
...
export async function POST(request: Request) {
  ...
  const endpoint = "/api/overtime/approve";
  ...
  try {
    ...
    const guarded = await runGuardedMutation(ctx, request, endpoint, async () => {
      const result = await approveOvertimeRequest(ctx, payload.requestId as string);
      if (!result.ok || !result.data) {
        return {
          status: mapServiceErrorStatus(result.error),
          body: { ok: false, error: sanitizeServiceError(result.error, "Overtime approval failed") }
        };
      }
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to approve overtime", route.requestId));
  }
}
