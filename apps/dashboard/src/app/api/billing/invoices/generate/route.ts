...
import { generateBillingInvoice } from "@emp/services/billing.service";
...
export async function POST(request: Request) {
  ...
  const endpoint = "/api/billing/invoices/generate";
  ...
  try {
    ...
    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await generateBillingInvoice(route.ctx!, {
      ...
      if (!result.ok) {
        return {
          ...
          body: { ok: false, error: sanitizeServiceError(result.error, "Unable to generate invoice") }
        };
      }
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to generate invoice", route.requestId));
  }
}
