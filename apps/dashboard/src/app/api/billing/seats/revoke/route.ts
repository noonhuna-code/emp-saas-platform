...
import { revokeCompanySeat } from "@emp/services/billing.service";
...
export async function POST(request: Request) {
  ...
  const endpoint = "/api/billing/seats/revoke";
  ...
  try {
    ...
    const body = (await request.json()) as RevokeSeatBody;
    ...
    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await revokeCompanySeat(route.ctx!, { userProfileId });

      if (!result.ok) {
        return {
          status: mapServiceErrorStatus(result.error),
          body: { ok: false, error: sanitizeServiceError(result.error, "Unable to revoke seat") }
        };
      }
    ...
    return finalizeRoute(route, endpoint, handleRouteError(error, "Unable to revoke seat", route.requestId));
  }
}
