...
import { listWorkspaceNotifications, markWorkspaceNotificationsRead } from "@emp/services/employee-workspace.service";
...
export async function GET(request: Request) {
  ...
  const endpoint = "/api/workspace/notifications";
  ...
  try {
    ...
    if (!result.ok) {
      return finalizeRoute(
        route,
        endpoint,
        jsonError(sanitizeServiceError(result.error, "Unable to load notifications"), mapServiceErrorStatus(result.error), route.requestId)
      );
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await markWorkspaceNotificationsRead(route.ctx!, {
      ...
      if (!result.ok) {
        return {
          ...
          body: { ok: false, error: sanitizeServiceError(result.error, "Unable to mark notifications as read") }
        };
      }
