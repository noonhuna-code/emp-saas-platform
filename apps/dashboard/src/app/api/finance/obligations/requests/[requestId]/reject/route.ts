...
import { rejectFinancialObligationRequest } from "@emp/services/financial-obligations.service";
...
export async function POST(
  ...
  const endpoint = "/api/finance/obligations/requests/[requestId]/reject";
  ...
  try {
    ...
    const guarded = await runGuardedMutation(route.ctx, request, endpoint, async () => {
      const result = await rejectFinancialObligationRequest(route.ctx!, requestId.trim(), body.reason);
