...
import { rejectAttendanceCorrection } from "@emp/services/attendance.service";
...
export async function POST(request: Request) {
  ...
  const endpoint = "/api/attendance/reject";
  ...
  try {
    ...
    const body = (await request.json()) as { correctionId?: string; reason?: string };
    ...
    const guarded = await runGuardedMutation(ctx, request, endpoint, async () => {
      const result = await rejectAttendanceCorrection(ctx, correctionId, reason);
      if (!result.ok) {
        return {
    ...
    return finalizeRoute(route, endpoint, jsonError("Attendance correction rejection failed", 500, route.requestId));
  }
}
