-- ============================================
-- Migration: 20260226120238_mark_billing_invoice_paid_atomic_lock_order_and_reason.sql
-- Purpose : Enforce subscription->invoice lock order and mandatory approval reason
...
begin
  ...
  v_reason := nullif(trim(coalesce(p_approval_reason, '')), '');
  if v_reason is null then
    raise exception using errcode = 'P0001', message = 'APPROVAL_REASON_REQUIRED';
  end if;

  select bi.subscription_id
    into v_invoice_subscription_id
  from public.billing_invoices bi
  where bi.company_id = p_company_id
    and bi.id = p_invoice_id
  limit 1;

  if v_invoice_subscription_id is null then
  ...
  where cs.company_id = p_company_id
    and cs.id = v_invoice_subscription_id
  limit 1
  for update;
  ...
  if v_subscription.cancellation_requested_at is not null then
    raise exception using errcode = 'P0001', message = 'CANCELLATION_REQUESTED';
  end if;
  ...
  v_provider := lower(nullif(trim(coalesce(p_payment_provider, '')), ''));
  ...
  values (
    ...
    v_provider,
    'manual_review',
    jsonb_build_object('paid_at', coalesce(p_paid_at, now()), 'approval_reason', v_reason),
    v_actor_profile_id,
    now()
  );
