-- ============================================
-- Migration: 20260226120229_run_billing_overdue_scheduler.sql
-- Purpose : Invoice overdue and subscription past_due correction engine
...
begin
  ...
  if v_invoice.status = 'pending' and current_date > v_invoice.due_date then
    update public.billing_invoices
       set status = 'overdue',
           updated_by = coalesce(v_actor_profile_id, updated_by),
           updated_at = now()
  ...
  v_grace_end := (v_invoice.due_date::timestamptz + interval '3 days');
  if v_subscription.status <> 'canceled'
     and v_subscription.status <> 'past_due'
     and current_timestamp > v_grace_end
     and v_invoice.status in ('pending', 'under_review', 'overdue') then
    ...
    perform public.write_billing_audit_event(
      ...
      v_subscription.id,
      jsonb_build_object('grace_ends_at', v_grace_end, 'invoice_id', v_invoice.id)
    );
  end if;
